    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_OPEN_LOCK:
            {
				...
				// zhang hong chao, no fail again after 3.1.0
				if (skillId != SKILL_NONE && skillId != SKILL_HERBALISM && skillId != SKILL_MINING && SKILL_SKINNING && (!m_selfContainer || ((*m_selfContainer) != this)))
                {
