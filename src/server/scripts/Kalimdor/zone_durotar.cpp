...
#define TEMP_OBJECT 174795  
#define TEMP_OBJECT_2 174797
...
public:
	...
	struct npc_lazy_peonAI : public ScriptedAI
	{
		...
		void SpellHit(Unit* caster, const SpellInfo* spell) override
		{
			...
			me->RemoveAllAuras();
			work = 2;
			if (GameObject* Lumberpile = me->FindNearestGameObject(TEMP_OBJECT, 20)) {
				me->SetWalk(false);
				me->GetMotionMaster()->MovePoint(1, Lumberpile->GetPositionX(), Lumberpile->GetPositionY(), Lumberpile->GetPositionZ());
			}
		}
		...
		void UpdateAI(uint32 diff) override
		{
			...
			if (work == 1) {
				...
				if (RebuffTimer < diff) {
					if (kicktime < 3) {
						if (GameObject* Lumberpile = me->FindNearestGameObject(TEMP_OBJECT_2, 30)) {
							me->RemoveAllAuras();
							me->SetWalk(true);
							me->GetMotionMaster()->MovePoint(3, Lumberpile->GetPositionX() - 1, Lumberpile->GetPositionY(), Lumberpile->GetPositionZ());
							kicktime++;
							work = 2;
						}
					}
